...
AmclNode::pf_resample(pf_t * pf){
  ...
  while(new_particle_set->sample_count < pf->max_samples)
  {
    ...
    if(drand48() < w_diff){
      ...
      */
      ...
      while (i < old_particle_set->sample_count && cd_old_set[i+1] < x) {
        i++;
      }
    ...
    */
    if (new_particle_set->sample_count > M) break;
  }
  ...
  */
  for (int j = 0; j < new_particle_set->sample_count; j++) {
    new_particle_set->samples[j].weight /= total_weight;
  }
